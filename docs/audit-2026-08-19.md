...
read real time directly: `cutoff_unix_ms` in `src/app/state.cpp:cutoff_unix_ms` (drives
`analytics`/`summary_report`/`focus_summary_for_window` windows) and *all* of Storage's
stamps (`unix_now_ms`, `src/storage/storage.cpp:unix_now_ms` — one function since ADR-0007
collapsed the RFC3339/seconds pair this named). Consequence: a
...
(it's a lambda parameter already — `review_window_cutoff(window, since, cutoff_unix_ms)`), then
